...
interface UsuarioRepostory : CrudRepository<UsuarioEntity, Int> {
    @Query(value = "SELECT * FROM incidentes.usuario WHERE email=?1", nativeQuery = true)
    fun findByEmail(email: String): UsuarioEntity?

    @Query(value = "SELECT * FROM incidentes.usuario WHERE email=?1 AND contrasenia=?2", nativeQuery = true)
    fun findByEmailAndPassword(email: String, password: String): UsuarioEntity?

    @Query(value = "SELECT * FROM incidentes.usuario WHERE token=?1", nativeQuery = true)
